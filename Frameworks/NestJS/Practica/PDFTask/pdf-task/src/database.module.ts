...
const databaseProvider = { //Define un proveedor que se encargará de establecer la conexión a la base de datos.
  ...
  useFactory: async () => { //Usa una fábrica asíncrona para crear y devolver la instancia de la conexión.
    const client = new Client({ //Crea una nueva instancia del cliente de PostgreSQL con los detalles de conexión.
      user: 'soporte',
      host: 'localhost', 
      database: 'db_prueba',
      password: 'soporte',
    ...
    return client; //Retorna la instancia del cliente de PostgreSQL que estará conectada a la base de datos.
  },
};
